...
<form name="sgs2fa_form" id="loginform" action="<?php echo $args['redirect_to']; ?>" method="post">
	...
	<?php
	include_once SG_Security\DIR . '/templates/partials/2fa-qr-secret.php';

	if ( ! empty( $args['backup_codes'] ) ) { include SG_Security\DIR . '/templates/partials/new-backup-codes.php'; }
	?>
	
	<button id="saved_codes" <?php if ( ! empty( $args['backup_codes'] ) ) { ?> disabled="disabled" <?php } ?> href="<?php echo $args['redirect_to']; ?>" class="button button-primary"><?php esc_html_e( 'Continue', 'sg-security' ); ?></button>
</form>
